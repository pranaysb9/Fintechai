...
// ──────────────────────────────────────────────
async function getAIResponse(context: string, userQuestion: string): Promise<ReadableStream> {
  ...
  try {
    ...
    const fallbackText = "The AI service is currently unavailable. Based on the data, the stock appears stable. (Fallback response).";
...
`;
    ...
    // Fallback with proper SSE format
    const fallbackStr = 'An error occurred. Based on available data, the stock is trading near its daily range with moderate volatility.';
